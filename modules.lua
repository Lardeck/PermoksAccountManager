...
function ModuleMixin:Init(moduleName, payload)
    ...
    self.unitEvents = payload.unitEvents
	...
	for row_identifier, row in pairs(rows) do
		if row.version == false or row.version == WOW_PROJECT_ID then
			...
			if enums[row_identifier] then
				PermoksAccountManager:Print(
					"Please use another identifier for",
					module.name,
					row_identifier,
					". Module",
					enums[row_identifier].name,
					"already uses it."
				)
			else
...
local function RegisterModuleEvent(event, unit)
	functions[event] = functions[event]
		or function(charInfo, ...)
	...
	pcall(function()
        if unit then
            modulesEventFrame:RegisterUnitEvent(event, unit)
        else
...
local function AddUnitEvents(moduleEvents)
	if not moduleEvents then
		return
	end

	for eventString, v in pairs(moduleEvents) do
		local event, unit = strsplit(",", eventString)
		events[event] = events[event] or {}

		if type(v) == "table" then
			for _, func in pairs(v) do
				events[event][func] = false
			end
		else
			events[event][v] = false
		end

		RegisterModuleEvent(event, unit)
	end
end
...
function PermoksAccountManager:LoadModule(moduleName, module)
	...
	AddUnitEvents(module.unitEvents)
