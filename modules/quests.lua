...
local function UpdateAllRetailQuests(charInfo)
...
local function UpdateAllBCCQuests(charInfo)
	local self = PermoksAccountManager

	charInfo.questInfo = charInfo.questInfo or default
	charInfo.completedDailies = {}
			...
			if reset == "daily" and isComplete then
				if info.unique then
					if not charInfo.completedDailies[info.key] then
						...
						charInfo.completedDailies[info.key] = true
					end
				else
...
local function UpdateAllHiddenQuests(charInfo)
	local self = PermoksAccountManager
	if not charInfo.questInfo then UpdateAllQuests(charInfo) end
...
local function HiddenQuestTimer(charInfo)
	timer = timer or C_Timer.NewTimer(1, function() UpdateAllHiddenQuests(charInfo) timer = nil end)
end
...
local function UpdateBCCQuest(charInfo, questID)
	local self = PermoksAccountManager
	if not charInfo.questInfo then UpdateAllBCCQuests(charInfo) end
...
local function UpdateRetailQuest(charInfo, questID)
	local self = PermoksAccountManager
	if not charInfo.questInfo then UpdateAllRetailQuests(charInfo) end
