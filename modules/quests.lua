...
local labelRows = {
	...
	dungeon_weekly = {
		...
		isComplete = function(alt_data)
			return alt_data.questInfo
				and alt_data.questInfo.weekly
				and PermoksAccountManager:GetNumCompletedQuests(alt_data.questInfo.weekly.dungeon_quests) == 1
		end,
		group = "resetWeekly",
	...
	pvp_weekly = {
		...
		isComplete = function(alt_data)
			return alt_data.questInfo
				and alt_data.questInfo.weekly
				and PermoksAccountManager:GetNumCompletedQuests(alt_data.questInfo.weekly.pvp_quests) == 1
		end,
		group = "resetWeekly",
		version = WOW_PROJECT_MAINLINE,
	},
	...
	weekend_event = {
		...
		isComplete = function(alt_data)
			return alt_data.questInfo
				and alt_data.questInfo.weekly
				and PermoksAccountManager:GetNumCompletedQuests(alt_data.questInfo.weekly.weekend_event) == 1
		end,
		group = "resetWeekly",
		version = WOW_PROJECT_MAINLINE,
	},
	...
	korthia_dailies = {
		...
		isComplete = function(alt_data)
			return alt_data.questInfo
				and alt_data.questInfo.daily
				and PermoksAccountManager:GetNumCompletedQuests(alt_data.questInfo.daily.visible.korthia_dailies)
					>= 3
		end,
		group = "resetDaily",
		version = WOW_PROJECT_MAINLINE,
	...
	riftbound_cache = {
		...
		isComplete = function(alt_data)
			return alt_data.questInfo
				and alt_data.questInfo.daily
				and PermoksAccountManager:GetNumCompletedQuests(alt_data.questInfo.daily.riftbound_cache) >= 4
		end,
		group = "resetDaily",
		version = WOW_PROJECT_MAINLINE,
	},
	relic_creatures = {
		...
		isComplete = function(alt_data)
			return alt_data.questInfo
				and alt_data.questInfo.daily
				and PermoksAccountManager:GetNumCompletedQuests(alt_data.questInfo.daily.relic_creatures) >= 15
		end,
		group = "resetDaily",
		version = WOW_PROJECT_MAINLINE,
	},
	dailyQuestCounter = {
		...
		data = function(alt_data)
			return alt_data.completedDailies
				and PermoksAccountManager:CreateFractionString((alt_data.completedDailies.num or 0), 25)
		end,
		group = "resetDaily",
	...
	relic_gorger = {
		...
		isComplete = function(alt_data)
			return alt_data.questInfo
				and alt_data.questInfo.daily
				and PermoksAccountManager:GetNumCompletedQuests(alt_data.questInfo.daily.relic_gorger) >= 4
		end,
		group = "resetDaily",
		version = WOW_PROJECT_MAINLINE,
	},
	helsworn_chest = {
		...
		isComplete = function(alt_data)
			return alt_data.questInfo
				and alt_data.questInfo.daily
				and PermoksAccountManager:GetNumCompletedQuests(alt_data.questInfo.daily.helsworn_chest) >= 1
		end,
		group = "resetDaily",
		version = WOW_PROJECT_MAINLINE,
	},
	assault_vessels = {
		...
		isComplete = function(alt_data)
			return alt_data.questInfo
				and alt_data.questInfo.weekly
				and PermoksAccountManager:GetNumCompletedQuests(alt_data.questInfo.weekly.assault_vessels) >= 2
		end,
		group = "resetWeekly",
		version = WOW_PROJECT_MAINLINE,
	},
	rift_vessels = {
		...
		isComplete = function(alt_data)
			return alt_data.questInfo
				and alt_data.questInfo.weekly
				and PermoksAccountManager:GetNumCompletedQuests(alt_data.questInfo.weekly.hidden.rift_vessels) >= 3
		end,
		group = "resetWeekly",
		version = WOW_PROJECT_MAINLINE,
	},
	adamant_vault_conduit = {
		...
		isComplete = function(alt_data)
			return alt_data.questInfo
				and alt_data.questInfo.weekly
				and PermoksAccountManager:GetNumCompletedQuests(
						alt_data.questInfo.weekly.hidden.adamant_vault_conduit
					)
					== 1
		end,
		group = "resetWeekly",
	...
	maw_dailies = {
		...
		isComplete = function(alt_data)
			return alt_data.questInfo
				and alt_data.questInfo.daily
				and PermoksAccountManager:GetNumCompletedQuests(alt_data.questInfo.daily.maw_dailies) >= 2
		end,
		group = "resetDaily",
		version = WOW_PROJECT_MAINLINE,
	},
	...
	anima_weekly = {
		...
		isComplete = function(alt_data)
			return alt_data.questInfo
				and alt_data.questInfo.weekly
				and PermoksAccountManager:GetNumCompletedQuests(alt_data.questInfo.weekly.visible.anima_weekly) == 1
		end,
		group = "resetWeekly",
		version = WOW_PROJECT_MAINLINE,
	},
	maw_souls = {
		...
		isComplete = function(alt_data)
			return alt_data.questInfo
				and alt_data.questInfo.weekly
				and PermoksAccountManager:GetNumCompletedQuests(alt_data.questInfo.weekly.visible.maw_souls) == 1
		end,
		group = "resetWeekly",
		version = WOW_PROJECT_MAINLINE,
	},
	korthia_weekly = {
		...
		isComplete = function(alt_data)
			return alt_data.questInfo
				and alt_data.questInfo.weekly
				and PermoksAccountManager:GetNumCompletedQuests(alt_data.questInfo.weekly.visible.korthia_weekly)
					== 1
		end,
		group = "resetWeekly",
		version = WOW_PROJECT_MAINLINE,
	...
	tormentors_weekly = {
		...
		isComplete = function(alt_data)
			return alt_data.questInfo
				and alt_data.questInfo.weekly
				and PermoksAccountManager:GetNumCompletedQuests(alt_data.questInfo.weekly.hidden.tormentors_weekly)
					>= 1
		end,
		group = "resetWeekly",
		version = WOW_PROJECT_MAINLINE,
	...
	tormentors_locations = {
		...
		isComplete = function(alt_data)
			return alt_data.questInfo
				and alt_data.questInfo.weekly
				and PermoksAccountManager:GetNumCompletedQuests(alt_data.questInfo.weekly.tormentors_locations) >= 1
		end,
		group = "resetWeekly",
		version = WOW_PROJECT_MAINLINE,
	},
	maw_assault = {
		...
		isComplete = function(alt_data)
			return alt_data.questInfo
				and alt_data.questInfo.weekly
				and PermoksAccountManager:GetNumCompletedQuests(alt_data.questInfo.weekly.hidden.assault) >= 2
		end,
		group = "resetWeekly",
		version = WOW_PROJECT_MAINLINE,
	},
	battle_plans = {
		...
		isComplete = function(alt_data)
			return alt_data.questInfo
				and alt_data.questInfo.weekly
				and PermoksAccountManager:GetNumCompletedQuests(alt_data.questInfo.weekly.battle_plans) == 1
		end,
		group = "resetWeekly",
		version = WOW_PROJECT_MAINLINE,
	},
	korthia_supplies = {
		...
		isComplete = function(alt_data)
			return alt_data.questInfo
				and alt_data.questInfo.weekly
				and PermoksAccountManager:GetNumCompletedQuests(alt_data.questInfo.weekly.korthia_supplies) == 1
		end,
		group = "resetWeekly",
		version = WOW_PROJECT_MAINLINE,
	},
	...
	wrath = {
		...
		isComplete = function(alt_data)
			return alt_data.questInfo
				and alt_data.questInfo.weekly
				and PermoksAccountManager:GetNumCompletedQuests(alt_data.questInfo.weekly.wrath) == 1
		end,
		group = "resetWeekly",
		version = WOW_PROJECT_MAINLINE,
	},
	hunt = {
		...
		isComplete = function(alt_data)
			return alt_data.questInfo
				and alt_data.questInfo.weekly
				and PermoksAccountManager:GetNumCompletedQuests(alt_data.questInfo.weekly.hunt) == 1
		end,
		group = "resetWeekly",
		version = WOW_PROJECT_MAINLINE,
	},
...
local function GetQuestInfo(questLogIndex)
	if not PermoksAccountManager.isRetail then
		local title, _, _, isHeader, _, _, frequency, questID, _, _, _, _, _, _, _, isHidden =
			...
			if not self.isBC then
				...
				if info.covenant and covenant == info.covenant then
					...
					if info.sanctum and charInfo.sanctumInfo then
						sanctumTier = charInfo.sanctumInfo[info.sanctum] and charInfo.sanctumInfo[info.sanctum].tier
							or 0
			...
			for key, _ in pairs(keys.hidden) do
				if self.quests[key] then
					for questID, questData in pairs(self.quests[key]) do
						...
						if
							warbandInfo
							and questData.warband
							and warbandInfo.questInfo[questType]
							and warbandInfo.questInfo[questType].hidden
							and warbandInfo.questInfo[questType].hidden[key]
							and not warbandInfo.questInfo[questType].hidden[key][questID]
						then
...
local function AddQuest(_, questID, questLogIndex, questInfo)
	local self = PermoksAccountManager
	local questLogIndex = questLogIndex
		or (self.isBC and GetQuestLogIndexByID(questID) or C_QuestLog.GetLogIndexForQuestID(questID))
...
local function UpdateCurrentlyActiveQuests(charInfo)
	local numQuests = C_QuestLog and C_QuestLog.GetNumQuestLogEntries and C_QuestLog.GetNumQuestLogEntries()
		or GetNumQuestLogEntries()
	local info
...
function PermoksAccountManager:CreateQuestString(questInfo, numDesired, replaceWithPlus, capAtDesired, useWarbandInfo)
	...
	if replaceWithPlus and numCompleted >= numDesired then
		...
		return self:CreateFractionString(capAtDesired and min(numDesired, numCompleted) or numCompleted, numDesired)
	end
end
...
function PermoksAccountManager:QuestTooltip_OnEnter(button, alt_data, column)
	if
		not alt_data
		or not alt_data.questInfo
		or not alt_data.questInfo[column.reset]
		or not alt_data.questInfo[column.reset][column.visibility]
	then
		return
	end
	...
	for questId, isComplete in pairs(info) do
		if isComplete and quests[questId] and quests[questId].name then
			completedByName[quests[questId].name] = completedByName[quests[questId].name]
				or { num = 0, total = quests[questId].total }
...
function PermoksAccountManager:CompletedQuestsTooltip_OnEnter(button, altData, column, key)
	if
		not altData
		or not altData.questInfo
		or not altData.questInfo[column.questType]
		or not altData.questInfo[column.questType][column.visibility]
	then
		return
	end
			...
			for questID, questInfo in pairs(quests) do
				...
				if name then
					if questInfo.achievementID and questInfo.criteriaID then
						local completed =
							select(3, GetAchievementCriteriaInfoByID(questInfo.achievementID, questInfo.criteriaID))
						local achievementString = completed
								and string.format("|cFF%s%s|r", COLOR_COMPLETED, column.achievementString)
							or string.format("|cFF%s%s|r", COLOR_NOT_COMPLETED, column.achievementString)
...
function PermoksAccountManager:KnowledgeTooltip_OnEnter(button, altData, column, key)
			...
			if skillLineID then
				...
				if skillLineID == prof1.skillLineID or skillLineID == prof2.skillLineID then
					...
					if questInfoTbl.item then
						tinsert(
							professionItems[questInfoTbl.skillLineID],
							{ questID, questInfoTbl.item, info[questID] }
						)
					end
				end
			end
	...
	if not next(professionItems) then
		return
	end
	...
	for skillLineID, info in pairs(professionItems) do
		...
		local maximum = column.professionRequired and column.professionRequired[skillLineID]
			or column.tooltipRequired
			or 2
		...
		if #info > 0 then
			...
			for _, itemInfo in ipairs(info) do
				...
				if item:IsItemDataCached() then
					tooltip:AddLine(
						string.format("%s%s", CreateSimpleTextureMarkup(item:GetItemIcon()), item:GetItemLink()),
						string.format("(%d)", questID),
						isComplete and completedString[1] or completedString[2]
					)
				else
...
function PermoksAccountManager:WOTLKDailyQuest_OnEnter(button, altData, labelRow, labelIdentifier)
	if
		(not altData or not altData.questInfo or not altData.questInfo[labelRow.questType])
		or not labelRow
		or not labelIdentifier
	then
		return
	end

	local info = altData[labelRow.questType][labelRow.visibility]
		and altData[labelRow.questType][labelRow.visibility][labelIdentifier]
	if not info then
		return
	end
