...
local function GetRewardItemLevel(id)
...
local function UpdateVaultInfo(charInfo, force)
	...
	for i, activityInfo in ipairs(activities) do
		...
		elseif activityInfo.type == Enum.WeeklyRewardChestThresholdType.Activities then
			...
			activityInfo.level = activityInfo.progress > 0 and GetLowestLevelInTopDungeonRuns(activityInfo.threshold)
				or activityInfo.level
		...
		if not vaultString then
			...
			if activityInfo.threshold > activityInfo.progress or i == 3 then
				if #difficulties > 0 then
					vaultString = string.format(
						"%s (%s)",
						self:CreateFractionString(activityInfo.progress, activityInfo.threshold),
						table.concat(difficulties, "\124\124")
					)
				else
...
function PermoksAccountManager.VaultTooltip_OnEnter(button, altData, labelRow)
	...
	for i, activityInfo in pairs(vaultInfo) do
		local rewardItemLevel = activityInfo.itemLevel
		...
		if activityInfo.type == Enum.WeeklyRewardChestThresholdType.Activities then
			local difficultyName = activityInfo.level
				and activityInfo.progress >= activityInfo.threshold
				and "+" .. activityInfo.level

			tooltip:AddLine(
				i .. ". Reward:",
				difficultyName or "-",
				"|",
				rewardItemLevel or self:CreateQuestString(activityInfo.progress, activityInfo.threshold)
			)
		elseif activityInfo.type == Enum.WeeklyRewardChestThresholdType.Raid then
			local difficultyName = activityInfo.level
				and activityInfo.progress >= activityInfo.threshold
				and DifficultyUtil.GetDifficultyName(activityInfo.level)
