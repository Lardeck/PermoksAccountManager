...
local labelRows = {
	mythics_done = {
		...
		data = function(alt_data)
			return alt_data.instanceInfo and PermoksAccountManager:CreateDungeonString(alt_data.instanceInfo.dungeons)
				or "-"
		end,
		group = "dungeons",
	...
	the_voidspire = {
		label = function()
			return PermoksAccountManager.raids[2912].name or "Voidspire"
		end,
		id = 2912,
		type = "raid",
		key = "the_voidspire",
	...
	the_dreamrift = {
		label = function()
			return PermoksAccountManager.raids[2939].name or "Dreamrift"
		end,
		id = 2939,
		type = "raid",
		key = "the_dreamrift",
	...
	march_on_queldanas = {
		label = function()
			return PermoksAccountManager.raids[2913].name or "March"
		end,
		id = 2913,
		type = "raid",
...
local function UpdateInstanceInfo(charInfo)
	...
	for i = 1, GetNumSavedInstances() do
		...
		name, _, _, difficulty, locked, extended, _, _, _, difficultyName, numEncounters, encounterProgress =
		...
		if locked or extended then
			if self.raids[mapID] or (self.isCata and self.raids[name]) then
				...
				instanceInfo.raids[info.englishID][difficulty] = instanceInfo.raids[info.englishID][difficulty]
					or {
						key = info.englishID,
			...
			elseif
				(self.dungeons[mapID] and difficulty == 23) or (self.isCata and self.dungeons[name] and difficulty == 2)
			then
			...
			elseif self.customRaids and self.customRaids[name] then
				...
				instanceInfo.customRaids[info.englishID][difficulty] = instanceInfo.customRaids[info.englishID][difficulty]
					or {
						key = info.englishID,
						difficulty = difficultyName,
						numEncounters = numEncounters,
...
function PermoksAccountManager:CreateRaidString(savedInfo, hideDifficulty)
	...
	if self.isCata then
		...
		for difficulty in
			PermoksAccountManager.spairs(savedInfo, function(_, a, b)
				return a < b
			end)
		do
			...
			if hasSingularLockout then
				...
				difficultyString = string.format(
					"%s %s",
					PermoksAccountManager.raidDifficultyLabels[difficulty],
					self:CreateFractionString(raidInfo.defeatedEncounters, raidInfo.numEncounters)
				)
			end

			raidString =
				string.format("%s%s%s", raidString, difficultyString, difficulty == highestDifficulty and "" or " ")
		end
	else
		...
		raidString = string.format(
			"%s%s",
			self:CreateFractionString(raidInfo.defeatedEncounters, raidInfo.numEncounters),
			hideDifficulty and "" or raidDifficulty
		)
	end
...
function PermoksAccountManager:DungeonTooltip_OnEnter(button, alt_data)
	...
	for key, value in
		self.spairs(self.dungeons, function(t, a, b)
			return t[a] < t[b]
		end)
	do
...
local function RetailRaid_OnEnter(tooltip, altData, dbInfo, raidInfo)
	...
	for difficulty, info in
		self.spairs(raidInfo, function(_, a, b)
