...
local labelRows = {
	...
	honorBCC = {
		label = function()
			return PermoksAccountManager.db.global.currencyInfo[1901]
					and PermoksAccountManager.db.global.currencyInfo[1901].name
				or "Honor"
		end,
		type = "currency",
		key = 1901,
	...
	arenaPoints = {
		label = function()
			return PermoksAccountManager.db.global.currencyInfo[1900]
					and PermoksAccountManager.db.global.currencyInfo[1900].name
				or "Arena Points"
		end,
		type = "currency",
		key = 1900,
...
local function UpdateAllCurrencies(charInfo)
	...
	for currencyType, offset in pairs(self.currency) do
		...
		if info then
			...
			currencyInfo[currencyType].maxQuantity = info.maxQuantity and info.maxQuantity > 0 and info.maxQuantity
				or nil
			...
			self.db.global.currencyInfo[currencyType].maxQuantity = info.maxQuantity
					and info.maxQuantity > 0
					and info.maxQuantity
				or self.db.global.currencyInfo[currencyType].maxQuantity
		end
	end
end
	...
	for treeID, treeInfo in pairs(self.currencyTrees) do
		...
		if treeCurrencyInfos then
			for _, treeCurrencyInfo in ipairs(treeCurrencyInfos) do
				currencyInfo[treeCurrencyInfo.traitCurrencyID] = charInfo.currencyInfo[treeCurrencyInfo.traitCurrencyID]
					or {}
...
local function UpdateWarbandAltCurrency(warbandCurrencyInfo, newWarbandCurrencyInfo, currencyType)
	if not warbandCurrencyInfo then
		return
	end

	warbandCurrencyInfo[currencyType] = warbandCurrencyInfo[currencyType]
		or { name = C_CurrencyInfo.GetCurrencyInfo(currencyType).name }

	warbandCurrencyInfo[currencyType].currencyType = currencyType
	warbandCurrencyInfo[currencyType].altQuantity = newWarbandCurrencyInfo
			and SumWarbandCurrencies(newWarbandCurrencyInfo)
		or 0
end
...
local function UpdateCurrency(charInfo, currencyType, quantity, quantityChanged)
	...
	if self.isRetail and quantityChanged then
	...
	if customOptions or not quantityChanged then
		if not quantityChanged or customOptions.forceUpdate then
			...
			currencyInfo.maxQuantity = newCurrencyInfo.maxQuantity
					and newCurrencyInfo.maxQuantity > 0
					and newCurrencyInfo.maxQuantity
				or nil
		elseif customOptions.currencyUpdate and charInfo.currencyInfo[customOptions.currencyUpdate] then
			charInfo.currencyInfo[customOptions.currencyUpdate].quantity =
				C_CurrencyInfo.GetCurrencyInfo(customOptions.currencyUpdate).quantity
		end
	else
...
local function CreateValorString(labelRow, currencyInfo)
	...
	if info then
		...
		local maxQuantity = (info.maxQuantity and info.maxQuantity > 0 and info.maxQuantity)
			or (globalCurrencyInfo and globalCurrencyInfo.maxQuantity or 0)
		return string.format(
			"%s - %s",
			AbbreviateNumbers(info.quantity),
...
local function CreateTreeCurrencyString(labelRow, currencyInfo)
	local info = currencyInfo and currencyInfo[labelRow.key]
	if info then
		return PermoksAccountManager:CreateFractionString(
			(info.quantity or 0) + (info.spent or 0),
			labelRow.maxQuantity or 0
		)
	end
end
...
function PermoksAccountManager:CreateCurrencyString(
	currencyInfo,
	abbreviateCurrent,
	abbreviateMaximum,
	hideMaximum,
	...
	if not hideIcon and currencyIcon and options.currencyIcons then
		if customIcon then
			iconString = string.format(
				"\124T%s:%d:%d:%d:%d\124t",
				customIcon.path or currencyIcon,
				customIcon.height or 18,
				customIcon.width or 18,
				customIcon.xOffset or 0,
				customIcon.yOffset or 0
			)
		else
	...
	if
		...
		and (currencyInfo.quantity or 0) > currencyInfo.maxQuantity
	then
	...
	if
		not hideMaximum
		and (
			(currencyInfo.maxQuantity and currencyInfo.maxQuantity > 0)
			or (currencyInfo.maxWeeklyQuantity and currencyInfo.maxWeeklyQuantity > 0)
		)
	then
		currencyString = self:CreateFractionString(
			quantity,
...
function PermoksAccountManager.CurrencyTooltip_OnEnter(button, altData, labelRow)
	...
	if ((globalCurrencyInfo.maxQuantity or currencyInfo.maxQuantity) or 0) > currencyInfo.totalEarned then
		tooltip:AddLine(
			"Left:",
			(globalCurrencyInfo.maxQuantity or currencyInfo.maxQuantity) - currencyInfo.totalEarned
		)
	end
...
do
	...
	function PermoksAccountManager:StygianEmbersTooltip_OnEnter(button, alt_data)
		if
			not alt_data
			or not alt_data.raidActivityInfo
			or not alt_data.questInfo
			or not alt_data.questInfo.weekly
			or not alt_data.questInfo.weekly.visible
		then
			return
		end
		...
		local embersFromShapingFate = questInfo.visible.korthia_weekly and next(questInfo.visible.korthia_weekly) and 10
			or 0
		...
		local total = GetTotalNumRaidEmbers(embersFromRaidBosses)
			+ embersFromShapingFate
			+ embersFromWB
			+ embersFromNormalRaidTrash
			+ embersFromHeroicRaidTrash
