				...
				args = {
					...
					buttonTextWidth = {
						...
						set = function(info, value)
							...
							options.buttons.widthPerAlt =
								max(options[parentKey].buttonWidth, options.buttons.widthPerAlt)
							PermoksAccountManager:UpdateAnchorsAndSize("general", true)
						end,
						min = 80,
						max = 250,
			...
			test = {
				...
				args = {
					...
					sortByComparison = {
						...
						get = function(_)
							return PermoksAccountManager.db.global.options.characters.sortByLesser and "lesser"
								or "greater"
						end,
					},
				},
								...
								args = {
									name = {
										...
										validate = function(info, value)
											...
											elseif
												PermoksAccountManager.db.global.currentCategories[value:lower()].name
											then
									...
									name = {
										...
										validate = function(info, value)
											...
											elseif string.len(value) == 0 then
												return "Can't create an empty category."
											elseif
												PermoksAccountManager.db.global.options.customCategories[value:lower()].name
											then
												return "This category already exists."
											end
	...
	options.args.order = {
		...
		args = {
			...
			defaultCategoriesOrder = {
				...
				validate = function(info, value)
					local newOrder = tonumber(value)
					return (not newOrder and "Please insert a number.")
						or (newOrder <= 0 and "Please insert a number greater than 0")
						or true
				end,
				args = {},
			},
	...
	local function UpdateLabelTable()
		for key, info in pairs(PermoksAccountManager.labelRows) do
			if not info.hideOption then
				...
				labelTable[group] = labelTable[group]
					or {
...
function PermoksAccountManager:OptionsToString()
	local export = {
		internalVersion = self.db.global.internalVersion,
		custom = self.db.global.custom,
		options = self.db.global.options,
	}
